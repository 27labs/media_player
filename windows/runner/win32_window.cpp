...
bool Win32Window::Create(const std::wstring& title,
  ...
  constexpr unsigned int kUseWindowsDefault =
      static_cast<unsigned int>(CW_USEDEFAULT);
  const int x = origin.x == kUseWindowsDefault
                    ? CW_USEDEFAULT
                    : Scale(origin.x, scale_factor);
  const int y = origin.y == kUseWindowsDefault
                    ? CW_USEDEFAULT
                    : Scale(origin.y, scale_factor);
  const int width = size.width == kUseWindowsDefault
                        ? CW_USEDEFAULT
                        : Scale(size.width, scale_factor);
  const int height = size.height == kUseWindowsDefault
                         ? CW_USEDEFAULT
